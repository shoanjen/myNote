...
* 解释：由认证机构（CA）为公钥施加数字签名。
* 作用：降低遭到中间人攻击的风险。

## 密钥

* 解释：用于将较长的秘密（消息）变成较短的秘密（密钥）
...
* 密钥的种类
  ...
  2. 用于认证的密钥
     ...
     * 数字签名的密钥：私钥签名生成，公钥签名验证。

  3. 内容加密密钥：CEK（Content Encrypting Key）。

  4. 密钥加密密钥：KEK（Key Encrypting Key）。

  5. 会话密钥：每次通信只能使用一次的密钥。
...
* 解释：将前一个密文分组与当前明文分组的内容混合（XOR运算），然后进行加密。
...
* 解释：前一个密文分组返回到加密算法输入端。将前一个密文分组进行加密，然后与当前明文分   组进行XOR运算。
...
* 解释：一种通过将逐次累加的计数器进行加密来生成密钥流的流密码。
